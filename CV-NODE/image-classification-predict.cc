...
int writeFile(std::string content, const std::string log_file)
{
    std::ofstream myfile;
...
void PrintOutputResult(const std::vector<float> &data, const std::vector<std::string> &synset, const std::string log_file)
{
    ...
    sprintf(tmp, "Best Result: [%s] id = %d, accuracy = %.8f\n",
            synset[best_idx].c_str(), best_idx, best_accuracy);
    writeFile(tmp, log_file);
}
...
int main(int argc, char *argv[])
{
    if (argc < 3)
    {
        std::cout << "Usage: ./image-classification-predict apple.jpg result.log" << std::endl;
        return 0;
    }
    ...
    // Print Output Data
    PrintOutputResult(data, synset, log_file);
